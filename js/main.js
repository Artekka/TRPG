...
totalDamage_enemy = 0;
    ...
    if(totalDamage < Enemy.hitPoints || totalDamage_enemy < Player.hitPoints) {
	...
	if (hitCheck) {
        ...
        if (skillShot) {
          ...
          if (Player.class === "archer") {
			damageThisRound = Math.round(Player.damage);
      ...
      if (hitCheck_enemy) {
        ...
        if (skillShot_enemy) {
          ...
          if (Enemy.class === "axe") {
			damageThisRound_enemy = Math.round(enemyClass.damage - (Player.armor * 0.7)) * 2 * dmgRng(8,10);
          ...
          if (Enemy.class === "archer") {
			damageThisRound_enemy = Math.round(Enemy.damage);
		...
		i++;
		if (totalDamage_enemy >= Player.hitPoints) {
